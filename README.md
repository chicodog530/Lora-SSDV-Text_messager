# LoRa Digital Image & Text Transceiver

An off-grid, two-way image and text messenger built with ESP32 and UART LoRa modules. This project turns two ESP32s into autonomous walkie-talkies that serve their own Wi-Fi web app to your smartphone, allowing you to send photos and text messages over miles of distance without any internet or cellular connection.
...
*(Note: Depending on your specific ESP32 board, you can change the RX/TX pins at the top of the `transceiver_v2.ino` sketch).*
...
2. **Flash the Firmware:**
   - Open `transceiver_v2.ino` in the Arduino IDE.
...
5. **Open the Web App:**
   - Open your smartphone's web browser (Chrome/Safari).
...
## Customization
You can rename the Wi-Fi network directly from the web interface under the "Device Settings" card. The board will automatically reboot and start broadcasting the new network name.
